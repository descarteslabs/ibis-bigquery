...
def trans_array(t, context):
    return "ARRAY<{}>".format(ibis_type_to_bigquery_type(t.value_type, context))
...
def trans_struct(t, context):
    return "STRUCT<{}>".format(
        ", ".join(
            "{} {}".format(name, ibis_type_to_bigquery_type(dt.dtype(type), context))
...
@ibis_type_to_bigquery_type.register(dt.GeoSpatial, TypeTranslationContext)
def trans_geospatial(t, context):
    # we should be handling GEOMETRY vs GEOGRAPHY here
    return "GEOGRAPHY"
...
def fix_type(f):
    @wraps(f)
    def wrapper(self):
        # their implementation doesn't account for this
        if self._accept(dt.Tokens.GEOGRAPHY):
            srid = None
            if self._accept(dt.Tokens.SEMICOLON):
                self._expect(dt.Tokens.INTEGER)
                assert self.tok is not None
                srid = self.tok.value

            return dt.Geography(srid=srid)
        return f(self)
